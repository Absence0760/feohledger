...
- `DataTable.svelte` — `.grid-container > table`. Pass `columns={[{label,class?}]}` (or a `{#snippet header()}<tr>…</tr>{/snippet}` for select-all/sortable headers) + a `{#snippet body()}` that renders the `<tr>`/`<td>` rows. `isEmpty` + `empty` render the centred empty row (`colspan` auto from columns). Opt-in `fixed` (table-layout:fixed) and `stickyHeader` props. The container is a named, keyboard-focusable scroll region (`role="region"`, `tabindex="0"`, `aria-label` = the `ariaLabel` prop or `common.tableRegion`) so a table wider than its card can be panned without a mouse — see `ui-patterns.md` § Accessibility patterns.
...
- `SearchPicker.svelte` — the searchable, **server-paged** WAI-ARIA 1.2 combobox under `VendorPicker` and `InvoicePicker`; use one of those, or add a third thin wrapper, rather than building on it directly from a page. Generic over the option (`generics="T extends { id: string }"`): `load` fetches a page (`{search, page, page_size}` → `{items, total}`) and its **identity is the set** — the answer the picker holds is stamped with the `load` that produced it, so a new function (a new memo, a new vendor) is a new set and the old options are never drawn, committed or announced; bind the scope into it with `$derived`. `optionLabel` is the committed text, the `option` snippet draws a row (its text is the option's accessible name), `text` carries every string already localized (the component is i18n-agnostic, like `FieldWarning`), `hint` joins the count line in the input's description, and `preload` fetches the unfiltered first page while CLOSED so the resting field can say the set is empty or unreachable. The count line states how much of the matching set is on screen and never presents a failed load as an empty set. Escape is bound with `onkeydowncapture` so it closes the popup, not the enclosing `Modal` — do not "tidy" that to `onkeydown` (`docs/decisions.md` §146). The rules (count, keyboard, what Enter commits, what the count line says and when) are pure and unit-tested in `utils/searchPicker.ts` (`docs/decisions.md` §202).
- `VendorPicker.svelte` — the vendor field on every surface that has one (`/catalogs` ×2, `/contracts`, `/recurring`, `/vendor-statements`, `/discounts`, `/credit-memos`). A `SearchPicker` over `searchVendorOptions()`: filtering is server-side so a vendor on page 40 is reachable, and a count line states how much of the matching set is on screen. **Never re-add a `<select>` over a client-side vendor list** — that is the bug this replaced, and it made every vendor past the first 100 unselectable in all six places at once. `bind:value` is the vendor uuid; pass `selectedLabel` from the row's `vendor_name` in edit mode, `label` (or `ariaLabel` for an inline control), and `onselect` if you need the chosen option. Fetches only when opened, and stays silent at rest.
- `InvoicePicker.svelte` — the invoice field on both `/credit-memos` dialogs (Apply, and the create dialog's optional "Apply to invoice" link). A `SearchPicker` that always **preloads** — it fetches when its dialog opens, never on page mount — over a `load` the page binds to the scope: `api/creditMemos.ts::listInvoicesEligibleForMemo(memoId)` / `listInvoicesEligibleForNewMemo(vendorId, amount)`, which return exactly the invoices the matching application path will accept (`docs/decisions.md` §202). `emptyText` is required and caller-owned, because only the caller can say what its set's emptiness means ("no invoice can take this credit" is not "this tenant has no invoices"). Options read `<number> <amount>`, plus "{amount} left to credit" when earlier credits have eaten into the invoice (`utils/invoicePicker.ts::remainingToCredit`). **Never re-add a `<select>` over a client-walked invoice list** — that is what this replaced.
- `Badge.svelte` — **the** tinted-badge primitive, and the single owner of the `background: var(--<tone>-tint); color: var(--<tone>-on-tint)` recipe. `<Badge tone="accent|success|warning|danger|muted|neutral|erp" variant? title?>{label}</Badge>`. A caller names a *tone*, so it can't spell one wrong, and a tone that is later recalibrated moves in one place. `variant` is passed through as an extra class for **selector hooks only** (`.badge.approved`, `.badge.violation` — the e2e suite reads them); never give a variant a colour rule in the calling component, pick the tone instead. `neutral` is a flat `--bg` chip for the absence of a signal (cancelled / n-a), deliberately not a tint; `erp` is the one measured literal (purple shares no semantic with the five tones, so it stays here rather than becoming a palette token with one caller). **Sizing is fixed on purpose** — call sites varied padding by a pixel or two with no intent behind it. A pill that genuinely needs different metrics is a different component, not a prop: `ScreeningBadge` is the worked example (its own smaller sentence-case metrics, but the palette tokens for colour). Where a status is badged in more than one place, put a `STATUS_TONES: Record<Status, BadgeTone>` map beside the existing `*_STATUS_LABEL_KEYS` in the shared types module so the list page and its modal can't disagree — several did. **`BadgeTone` lives in `ui/badgeTone.ts`, not in the component** — import it from there (`import type { BadgeTone } from '$lib/components/ui/badgeTone'`). Plain `tsc`, which typechecks `tests-e2e/`, resolves a `.svelte` import through a shim with no named exports, so a type declared in `Badge.svelte` was a TS2614 for any e2e fixture whose `$lib/types` counterpart used it; `Badge.svelte` deliberately does not re-export it.
- `BrandMark.svelte` — the FeohLedger mark (feoh ᚠ as a split tally stick, `docs/decisions.md` §173) as an `<img>` of `static/logo-mark.svg`. `<BrandMark size={24} label? />`. Decorative by default (`alt=""`): beside the product name the name carries it, and on the auth cards the heading and `<title>` do; pass `label` only where it is the one thing naming the product in view (the collapsed sidebar rail). **Platform chrome only**: anything a white-label tenant sees picks its mark through `brand.mark` (`brandTheme.ts::brandMark`), which resolves to this only while the product still carries the platform's name. Used by `Sidebar`, `Landing`, `AuthShell`'s brand panel, `AuthCard`, the login, MFA, signup, verify and change-password pages, and the supplier portal's header and login card (through `portalBrand.mark`). The SVG is generated — change `assets/logo-render/gen_svg.py` and run `pnpm gen:icons`, never edit the file.
...
- `SettingsRail.svelte` — vertical, grouped, **URL-backed** section picker for a settings page. `<SettingsRail groups={[{label?, items:[{slug,label}]}]} active={section} label="Settings section" />`. Renders anchors to `?section=<slug>` (every other query parameter preserved, any stale `#hash` dropped) with `aria-current="page"` on the active one and `data-section-link="<slug>"` for specs; collapses behind a disclosure button below 60rem. Labels arrive already-localized, like `SecretReveal` — the component is i18n-agnostic so it can serve pages with their own key namespaces. The caller owns `active` (it reads `?section=` itself, so it can validate the slug and fall back). A group label is deliberately **not** a heading — it is hidden from assistive tech and re-attached as the list's accessible name, because the panels it links to use `<h2>` for their own titles. Used by `/organization` (fifteen panels, five groups) and `/profile` (seven, flat). **Distinct from `Tabs.svelte`** (local view state, buttons) **and `layout/SectionTabs.svelte`** (cross-route, driven by `nav.ts`) — `frontend/docs/ui-patterns.md` § Navigating a long page has the picking rule, and `docs/decisions.md` §188 the reasoning.
...
- `LinkedMessage.svelte` — a translated sentence with links inside it. `<LinkedMessage text={m('key')} links={{ token: { href, label } }} />`. The catalogue entry carries `{token}` markers; this splits the message around them (`i18n/segments.ts::splitTokens`) and renders one anchor per token, so the **translation** decides where each link falls. Use it instead of cutting the sentence into `…Pre`/`…Post` catalogue entries with the anchor written between them — fragments fix the link order, which German and Japanese both need to change, and a translator handed three of them can only reproduce English word order or mistranslate. An unknown `{token}` is left visible rather than swallowed (a stray marker gets reported; a silently dropped link does not), and `messages_parity.test.ts` already fails any locale whose placeholder set differs from English, so a translation cannot lose a link. Renders anchors it builds itself and binds the text as text — no `{@html}`. Used by the signup consent line (three links, `auth.signup.legalConsent`). For a link to a published legal document, take the label from `legal/pages.ts::legalTitle(path)` rather than typing the title.
...
- `GlAccountModal.svelte` — create one chart-of-accounts row (`POST /api/gl-accounts`, admin | ap_manager), or — given `account` — correct one (`PATCH /api/gl-accounts/{id}`). **Create states which chart the account will land in before creating it** — the backend decides that from the live `X-Entity-ID`, not from the body (consolidated ⇒ SHARED across every entity, an entity selected ⇒ that entity's own), the difference is otherwise invisible in the form, and the PATCH deliberately cannot move a row between charts afterwards. The hint renders only when `entityStore.multiEntity`, the switcher's own gate. Surfaces the backend's own 409 message verbatim — it names the code AND the chart it already exists in, which is exactly what the user has to change. **Edit** shows the code read-only with the reason (an invoice records it as text, so a rename would orphan every coded line — replace an account by creating the new one and retiring this one), keeps an ERP-written type outside the known four selectable so an untouched save cannot rewrite it, sends ONLY the changed fields, and renders the backend's refusal inline in `role="alert"` (`[data-testid="gl-account-save-error"]`) — the parent-cycle 422 names the account below, and the 403 names the view to edit a shared row from. Retire is not a field here: it is a row action on `/gl-accounts`.
...
- `MotionToggle.svelte` — the WCAG 2.2.2 pause control. `<MotionToggle bind:paused pauseLabel? playLabel? />`; the caller puts `data-motion={paused ? 'paused' : 'running'}` on its root and `app.css` pauses every animation beneath it.

**`auth/`** — pre-auth employee pages.
- `AuthShell.svelte` — split-screen frame: the page's form, plus a platform-branded panel (tagline, page-specific `panel` snippet, the tally render). `<AuthShell panelOnMobile?>{form}{#snippet panel()}…{/snippet}</AuthShell>`. Owns the shared field, submit-button and `.error` styling via `:global(:where(…))` under `.form-wrap` — zero added specificity, so a page's own scoped rule wins without hacks (signup's composite slug field, MFA's code input). DOM order is form then panel. Used by signup, login, MFA, verify, change-password and `AuthCard`. **Not** the supplier portal's sign-in, which carries tenant branding.
- `AuthCard.svelte` — heading + subtitle + error for the secondary pages (forgot / reset password), rendered inside `AuthShell`. `<AuthCard heading subtitle? error?>`. Keeps the `.auth-card` class as the e2e hook.
