...
- `Backdrop.svelte` — `aria-hidden` atmosphere: aurora blobs, ruled grid, generated grain, vignette. `<Backdrop fixed? dense? still? />`. `fixed` pins it to the viewport (the long landing page); `dense` tightens it for a narrow panel; `still` holds the aurora at rest (the auth pages, which carry no pause control — see Motion).
