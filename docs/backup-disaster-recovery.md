...
- **GDPR right-to-erasure** — the erasure process itself is separate and tracked in the privacy roadmap. Its interaction with backups belongs here, though, because DPA § 13 makes a customer-facing promise that rests entirely on this mechanism, and it needs three qualifications an auditor will ask for:
  ...
  - **`deploy/remove-tenant.sh` implements the deletion** (2026-09-16). It is the termination path end-to-end: it stops serving the tenant's host, runs `scripts/delete_tenant.py` in the api container (documents → tenant database → control-plane rows), and then deletes **every version and delete marker** of `pg/*/feoh_<slug>.dump` from this bucket. `--dry-run` inventories without changing anything.
    Two things about it are worth an auditor's attention:
    - It deletes **versions**, not just the current object. The hand-run loop this replaced (`aws s3 rm …` per date prefix) writes a *delete marker* and leaves the superseded version to expire 30 days later under `noncurrent_version_expiration` — the residue DPA § 13 discloses. Deleting the versions outright means that residue does not arise for a tenant removed this way. The DPA's disclosure stays as written: it is conservative rather than wrong, and it still describes what happens if anyone deletes by hand.
    - The object listing is filtered with a `/`-anchored suffix (`ends_with(Key, '/feoh_<slug>.dump')`), so removing `acme` cannot match a tenant called `not-acme`, and never matches `feohledger.dump`. The shared control-plane dump is deliberately left alone — see the bullet above; it is not selectively editable and ages out with the window.
    - **The confirmation is verified, not inferred.** After deleting, the script lists the bucket again and prints the customer confirmation only if nothing for that tenant remains. A failed AWS listing stops the run rather than reading as "nothing to delete", and a nightly dump written mid-run is caught and named. Either way the script is safe to re-run, including after the tenant's organisation is already gone.
...
| 2026-09-16 | The 60-day backup deletion stopped being a calendar obligation: `deploy/remove-tenant.sh` removes a terminated tenant's Caddy host, data and every version of its nightly dumps, and `scripts/delete_tenant.py` behind it removes the documents, tenant database and control-plane rows. Updated the erasure-from-backups qualifications accordingly — the third one no longer says nothing implements it | Founder |
