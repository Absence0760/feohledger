...
class UserResponse(BaseModel):
    ...
    # The caller's organization has closed password sign-in (`sso_only`), so
    # the password is not a step-up proof either. Filled from the SAME
    # predicate `login` refuses on and `_step_up_satisfied` drops the password
    # on (`api/auth._org_closes_password_sign_in`) — deliberately not the public
    # `/auth/{sso,saml}/config` echo, which reports `sso_only` only when the IdP
    # config resolves and so says "open" for a tenant whose broken config still
    # closes the password. The profile page reads it to stop offering a password
    # field for a factor change (docs/decisions.md §201).
