...
#      (assets/icon*.svg, assets/logo-mark.svg) and the web SVG copies.
...
# render_opaque: the same, flattened and forced to 8-bit RGB (PNG colour type 2).
# Without the explicit colour type ImageMagick may palette-optimise a small icon
# into an indexed PNG that still carries a tRNS chunk, and App Store Connect
# rejects any icon with alpha, indexed or not.
render_opaque() {
  local svg="$1" size="$2" out="$3"
  mkdir -p "$(dirname "$out")"
  inkscape "$svg" -w "$size" -h "$size" -o "$out" >/dev/null 2>&1
  ...
  echo "  ${out#"$REPO_ROOT"/} (${size}px, opaque)"
}
...
render_opaque "$ICON" 180 "$WEB/apple-touch-icon.png"
render "$MARK" 192 "$WEB/icon-192.png"
render "$MARK" 512 "$WEB/icon-512.png"
render_opaque "$ICON" 512 "$WEB/icon-maskable-512.png"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
for density in mdpi hdpi xhdpi xxhdpi xxxhdpi; do
  ...
  render_opaque "$BACKGROUND" "${ADAPTIVE[$density]}" "$RES/mipmap-$density/ic_launcher_background.png"
...
while read -r filename px; do
  render_opaque "$ICON" "$px" "$IOS/$filename"
