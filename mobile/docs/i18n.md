...
- **Invoice warnings** — the findings on the invoice-detail panel are composed
  ...
  - **The catalogue is not hand-authored.** Its codes and parameter kinds are
    transcribed from `frontend/src/lib/api/invoiceWarningMessages.generated.ts`
    — which `pnpm gen:warning-messages` generates from
    `backend/app/services/invoice_warning_catalog.py` — and
    `test/l10n/invoice_warning_messages_test.dart` reads that generated module
    and fails when the two hold different codes or different kinds. A new
    `WarningSpec` therefore reddens mobile CI, not just the web's. (The
    generator itself still emits only the TypeScript half; making it write the
    Dart/ARB half too is the remaining step, and the parity test is what makes
    its absence safe meanwhile.)
