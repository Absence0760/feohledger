...
- **Currency** — every money figure goes through `lib/utils/money.dart`
  ...
  wrong for every non-USD tenant. It takes an optional `locale` (the code
  drives the symbol, the locale drives grouping, separators and symbol
  placement — a EUR figure is `€1,234.50` to an `en` reader and
  `1.234,50 €` to a `de` one), but **nothing passes one yet**: no call site
  wires `LocaleStore` into it, and `Intl.defaultLocale` is never set, so
  numbers and dates still format against intl's default regardless of the
  picker. That gap is tracked in `docs/followups.md`; it is the one place this
  file's "every number, date and currency renders through the locale-aware
  helpers" is aspirational rather than true.
