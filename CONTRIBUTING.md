...
|---|---|---|
| **Web bundle budget** (`web-bundle-budget.yml`) | The frontend is static and served from S3 behind CloudFront, so bundle weight is paid by every cold visit and nothing else in CI notices it. Fails on a total or per-chunk ceiling. The total counts every chunk but only the largest lazy locale catalogue, since a visitor fetches one. | `cd frontend && PUBLIC_API_URL=http://localhost:8000 pnpm build`, then from the repo root run the workflow's `Measure + assert budget` script with `MAX_TOTAL_KB` and `MAX_LARGEST_CHUNK_KB` set to its values (it needs `jq`, and reads `frontend/build` plus the Vite manifest) |
